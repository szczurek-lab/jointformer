...
class Evaluator(CustomTrainer):
    ...
    def conditional_generation(
        ...
        if save_path is not None:
            ...
            "GeneratedSequence": generated_seqs
            })
            save_path = "./trained_models/qed/results/"
            sequences_df.to_csv(os.path.join(save_path, 'generated_sequences_1.csv'), index=False)
            print(f"Generated sequences saved in {save_path}/generated_sequences_1.csv")
