...
def _cluster_into_bins(eval_data, ref_data, num_clusters):
    """Clusters the union of the openai_eval points and returns the cluster distribution.
    ...
    Args:
      eval_data: NumPy array of openai_eval points from the distribution to be evaluated.
      ref_data: NumPy array of openai_eval points from the reference distribution.
...
def compute_prd_from_embedding(eval_data, ref_data, num_clusters=20,
    ...
    """Computes PRD openai_eval from sample embeddings.
    ...
    Args:
      ...
      num_angles: Number of angles for which to compute PRD. Must be in [3, 1e6].
                  The default value is 1001.
      num_runs: Number of independent runs over which to average the PRD openai_eval.
...
def plot(precision_recall_pairs, labels=None, out_path=None,
    ...
    Creates the PRD plot for the given openai_eval and stores the plot in a given path.
    Args:
